...
function Detail() {
  ...
  return <main className="page">
    ...
    <LifecycleNotice status={currentRun.status} />
...
function TaskDetail({ task, repair }: { task?: TaskSummary; repair?: Row }) {
  ...
  return <aside className="detail"><h2>{task.task_id}</h2><p className={statusClass(task.status)}>{task.status}</p><p>{task.summary || task.title || "No terminal summary yet."}</p>
    <dl><Metric name="Capability" value={task.capability} /><Metric name="Model profile" value={text(task.model_profile)} /><Metric name="Liveness" value={text(task.liveness)} /><Metric name="Dependencies" value={(task.dependencies ?? []).join(", ") || "—"} /></dl>
...
function Execution({ tasks, invocations, tools, validations, selected, onSelect }: { tasks: TaskSummary[]; invocations: Row[]; tools: Row[]; validations: Row[]; selected: string | null; onSelect: (id: string) => void }) {
  return <section className="stack"><h2>Tasks</h2><table><thead><tr><th>Task</th><th>Status</th><th>Model</th><th>Duration</th><th>Liveness</th><th>Summary</th></tr></thead><tbody>{tasks.map((task) => <tr className={selected === task.task_id ? "selected-row" : ""} key={task.task_id} onClick={() => onSelect(task.task_id)}><td>{task.task_id}</td><td><span className={statusClass(task.status)}>{task.status}</span></td><td>{text(task.model_profile)}</td><td>{duration(task.started_at, task.ended_at)}</td><td>{text(task.liveness)}</td><td>{text(task.summary)}</td></tr>)}</tbody></table><h2>Model invocations</h2><JsonTable rows={invocations} /><h2>Tool calls</h2><JsonTable rows={tools} /><h2>Validator results</h2><JsonTable rows={validations} /></section>;
}
...
function evidenceItems(artifacts: Row[], prompts: Row[], events: StreamEvent[], invocations: Row[], plan?: Row, lineage?: Row): EvidenceItem[] {
  const items: EvidenceItem[] = artifacts.map((artifact) => ({ id: `artifact:${text(artifact.sha256)}`, kind: "artifact", sha256: text(artifact.sha256), label: text(artifact.logical_name), detail: `${text(artifact.media_type)} · ${number(artifact.size_bytes)} B` }));
...
function EvidenceInspector({ item, content, loading, error }: { item: EvidenceItem | null; content?: ContentView; loading: boolean; error: unknown }) {
  ...
  if (!content?.available) return <p>This capture is unavailable under its stored <code>{content?.capture_level ?? "unknown"}</code> capture policy. The dashboard does not request or reconstruct unavailable content.</p>;
  return <><p><small>{content.media_type} · {number(content.byte_count)} B · {content.capture_level}</small></p>{content.truncated && <p className="warn">Stored content was truncated before it reached this dashboard.</p>}<pre>{typeof content.payload === "string" ? content.payload : JSON.stringify(content.payload, null, 2)}</pre></>;
}
